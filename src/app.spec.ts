...
describe("BEDLENDULE", () => {
  ...
  test("createSchedule", async () => {
    ...
    const data = {
      ...
      location: ["Zoom", "Skype", "Google Meet"][Math.floor(Math.random() * 3)],
    };
  ...
  test("deleteSchedule", async () => {
    const startTime = new Date().getTime() + 86400000*2;
    const finishTime = startTime + 3600000;
    const data = {
      uuid: doctorUUID,
      title: "Depression Therapist",
    ...
    const result = await deleteSchedule({ scheduleId: schedule.id, uuid:doctorUUID });
...
describe("createRequest", () => {
  it("should create a request with the given arguments", async () => {
    ...
    expect(createdRequest.patientUUID).toBe(requestArgs.uuid);
  });
...
describe("getOpeningRequests", () => {
  test("should return all requests with doctorTimeslot = null", async () => {
    ...
    expect(result).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ doctorTimeslot: [] }),
      ])
    );
  });
});
...
describe("bookTimeSlot", () => {
  test("should create a request and connect to the selected time slot", async () => {
    ...
    console.log("bookTimeSlot", result);
    expect(result.patientUUID).toBe(data.uuid);
    // expect(result.doctorTimeslot?[0].id).toBe(data.timeslotId);
  });
});
describe("deleteRequest", () => {
  test("should delete selected request id", async () => {
    ...
    const requestToDelete = await createRequest(requestArgs)
    const result = await deleteRequest({ id: requestToDelete.id });
    console.log("deleteRequest",result);
    expect(result.id).toBe(requestToDelete.id);
  });
});
...
describe("chooseDoctor", () => {
  test("should delete all timeslots,schedules except the selected one, update the selected requestID status to CHOSEN", async () => {
    ...
    const requestToChoose = await createRequest(requestArgs)
    const timeSlot = await acceptRequest({
      requestId: requestToChoose.id,
      uuid: doctorUUID,
      startTime: requestToChoose.startTime.toISOString(),
      finishTime: requestToChoose.finishTime.toISOString(),
    })
    const result = await chooseDoctor({
      requestId: requestToChoose.id,
      timeSlotId: timeSlot.id,
    });
    console.log(" chooseDoctor", result);
  });
});
